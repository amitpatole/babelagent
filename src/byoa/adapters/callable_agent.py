"""Wrap any Python callable as an :class:`~byoa.core.agent.Agent`."""
...
class CallableAgent:
    """Adapts a plain callable into a station worker.

    Sync callables run in a worker thread so they never block the event loop.
    The callable's return value becomes the next part's payload (unless it
    already returns a :class:`Part`).
    """
    ...
    async def run(self, part: Part, ctx: Context) -> Part:
        args, kwargs = bind_payload(part.payload, self._sig)
        if self._is_async:
