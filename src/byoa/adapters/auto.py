...
Hand it *anything* a user brought (a callable, an HTTP/OpenAPI URL, an MCP ref,
a framework agent, or an already-conforming Agent) and it returns a uniform
:class:`~byoa.core.agent.Agent`. Extensible: third parties register their own
matchers via :func:`register_adapter` or the ``byoa.adapters`` entry-point group.
"""
...
def _load_entrypoint_adapters() -> None:
    ...
    try:
        ...
        eps = entry_points(group="byoa.adapters")
