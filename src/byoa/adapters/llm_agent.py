"""Adapt an LLM provider (Anthropic / OpenAI / Ollama) as a station worker.
...
class LLM:
    ...
    The station's input payload is substituted into ``prompt`` via ``{input}``
    ...
    async def run(self, part: Part, ctx: Context) -> Part:
        message = self._render(part.payload)
        if self.provider == "anthropic":
            text = await self._anthropic(message)
        elif self.provider == "openai":
            text = await self._openai(message)
        elif self.provider == "ollama":
            text = await self._ollama(message)
        else:
            raise MissingDependencyError(f"provider {self.provider!r}", "cloud")
        return part.with_payload(text, station=self.name, model=self.model)
