...
Pass `after=[...]` and you are drawing a graph. Independent nodes run concurrently; a `join` waits for
its upstreams and receives their outputs keyed by node name.
...
`k`), or `optional` (proceed once everyone settled, pass or fail).
