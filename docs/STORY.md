...
An honest architecture document says what is missing. Today Babelagent has a tested core, the adapter layer with `adapt()`, and the command-line interface with the demo. It does not yet have the REST service or the MCP server that would let other programs and agents drive a graph remotely. It does not yet ship the A2A adapter that would turn a remote agent into a node, which is the interoperability move section 9 argues is the strongest position. It has a basic SSRF guard but has not been through its dedicated security pass, which is where request-size caps, connection and handler timeouts, constant-time authentication for the REST service, a fail-closed posture on any non-loopback bind, and several rounds of adversarial review will land. It is local-only by choice for now, so there is no published package, no public repository, and no hosted documentation. None of these gaps are accidental. They are the next steps, sequenced so that the machine is proven correct before it is exposed to the network and only then dressed up and shipped.
